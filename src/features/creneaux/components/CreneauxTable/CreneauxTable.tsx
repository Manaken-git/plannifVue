...
export function CreneauxTable({ items, searchTerm, onSearchChange, onEdit, onDelete }: Props) {
  const filtered = items.filter((item) => `${item.debut} ${item.fin}`.includes(searchTerm));
  const selectors = useMemo(() => ({ debut: (item: Creneau) => item.debut, fin: (item: Creneau) => item.fin }), []);
  const sorted = useSortableData(filtered, selectors);
  return <DataTable title="Créneaux horaires" count={items.length} searchTerm={searchTerm} onSearchChange={onSearchChange} searchPlaceholder="Rechercher une heure…">
    {filtered.length === 0 ? <EmptyState compact icon={<Clock3 size={20} />} title="Aucun créneau" description="Créez les plages horaires qui serviront de base au planning." /> : <TableScroll><table className="entity-table"><thead><tr><th><SortButton label="Début" columnKey="debut" sortConfig={sorted.sortConfig} onSort={sorted.requestSort} /></th><th><SortButton label="Fin" columnKey="fin" sortConfig={sorted.sortConfig} onSort={sorted.requestSort} /></th><th>Durée</th><th style={{ width: 90 }}>Actions</th></tr></thead><tbody>{sorted.items.map((item) => {
      const [dh, dm] = item.debut.split(':').map(Number); const [fh, fm] = item.fin.split(':').map(Number); const duration = Number.isFinite(dh + dm + fh + fm) ? Math.max(0, (fh * 60 + fm) - (dh * 60 + dm)) : 0;
      return <tr key={item.id ?? `${item.debut}-${item.fin}`}><td className="entity-table__primary">{shortTime(item.debut)}</td><td>{shortTime(item.fin)}</td><td><Badge tone="primary">{Math.floor(duration / 60)} h {duration % 60 ? `${duration % 60} min` : ''}</Badge></td><td><div className="entity-table__actions"><Button variant="icon" className="entity-table__action-edit" onClick={() => onEdit(item)} icon={<Edit2 size={14} />} /><Button variant="icon" className="entity-table__action-delete" onClick={() => item.id && onDelete(item.id)} icon={<Trash2 size={14} />} /></div></td></tr>;
